...
[  $(cat $SOURCESFILE | wc -c) -eq 1 ] && echo "<<< $SOURCESFILE is empty; exiting" && exit 1
...
case "$2" in
 ...
 "backports");;
  ...
  "") if [ "$1" != "default" -a  "$1" != "list-enabled" -a "$1" != "help"  ] ; then
   ...
   "default")  echo "Default selected"
	    ...
	    for suite in ${defaultSuites[@]};do
               for URI in ${uriArray[@]};do
                  ...
                  [ $SOURCES -eq 1 ] &&  echo deb-src $URI $suite $enabledComponents | tee -a $SOURCES
               done
            done            
              ;;
